...
function ImagePlaceholder({ isHarvest }) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px' }}>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M12 2C8.5 2 5.5 4.5 5 8C3.3 8.5 2 10.1 2 12C2 14.2 3.8 16 6 16H18C20.2 16 22 14.2 22 12C22 10 20.5 8.3 18.5 8C17.5 4.6 14.9 2 12 2Z" fill={color} fillOpacity="0.25" />
        <circle cx="12" cy="11" r="2.5" fill={color} fillOpacity="0.6" />
        <path d="M8 20C8 20 9 17 12 17C15 17 16 20 16 20" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeOpacity="0.5" />
      </svg>
    </div>
  );
}
...
function LogSkeleton() {
  return (
    <div style={{ background: 'white', padding: '14px', borderRadius: '18px', border: '1px solid rgba(27, 67, 50, 0.05)', display: 'flex', alignItems: 'center', gap: '12px' }}>
...
function LogCard({ log, idx }) {
  ...
  return (
    <div
      ...
      onMouseDown={e => { e.currentTarget.style.transform = 'scale(0.99)'; e.currentTarget.style.boxShadow = '0 1px 6px rgba(27, 67, 50, 0.06)'; }}
      onMouseUp={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = '0 2px 12px rgba(27, 67, 50, 0.04)'; }}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontSize: '12px', fontWeight: 700, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '3px' }}>{log.log_id}</p>
        <p style={{ fontSize: '10px', color: '#6B7280', fontWeight: 400 }}>{log.date}<span style={{ color: 'rgba(27,67,50,0.20)', margin: '0 5px' }}>·</span>{log.time}</p>
      </div>

      <button style={{ width: 34, height: 34, borderRadius: '10px', background: 'rgba(27, 67, 50, 0.05)', border: '1px solid rgba(27, 67, 50, 0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, cursor: 'pointer', transition: 'all 0.2s ease' }}>
...
export default function History() {
  ...
  const filtered = safeLogs.filter(log => {
    ...
    const isHarvest = statusStr.toLowerCase().includes('harvest') && !statusStr.toLowerCase().includes('not');
    const matchesSearch = logIdStr.toLowerCase().includes(searchQuery.toLowerCase()) || statusStr.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesFilter = filterMode === 'all' || (filterMode === 'harvest' && isHarvest) || (filterMode === 'not' && !isHarvest);
  ...
  return (
    <div style={{ background: '#F8F9FA', minHeight: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', top: -30, right: -20, width: 130, height: 130, borderRadius: '50%', background: 'rgba(255,255,255,0.04)', pointerEvents: 'none' }} />
        ...
        <div className="flex justify-between items-center relative z-10">
          ...
          <button style={{ background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '12px', padding: '9px' }}>
        ...
        <div className="flex gap-2" style={{ marginTop: '16px' }}>
          {[
            ...
            <button
              ...
              style={{ flex: 1, padding: '10px 8px', borderRadius: '12px', border: filterMode === mode ? '1.5px solid rgba(255,255,255,0.35)' : '1px solid rgba(255,255,255,0.08)', background: filterMode === mode ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.06)', backdropFilter: 'blur(8px)', cursor: 'pointer', transition: 'all 0.2s ease', textAlign: 'center' }}
            >
              <div style={{ fontSize: '18px', fontWeight: 800, color, lineHeight: 1 }}>{value}</div>
              <div style={{ fontSize: '9px', fontWeight: 600, color: 'rgba(255,255,255,0.45)', marginTop: '2px', letterSpacing: '0.3px', textTransform: 'uppercase' }}>{label}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '0px 16px 32px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {isLoading ? (
          <><LogSkeleton /><LogSkeleton /><LogSkeleton /><LogSkeleton /></>
        ) : filtered.length === 0 ? (
