...
export default function LiveDetection() {
  ...
  // 1. Start Camera (Only if not in gallery mode)
  useEffect(() => {
  ...
  // NEW: 5. Handle Image Upload from Gallery
  const handleFileUpload = async (event) => {
    const file = event.target.files[0];
    if (!file) return;

    // Switch to gallery mode & show loading state
    setIsScanning(false);
    ...
    // Send the actual file to Python
    const formData = new FormData();
    ...
    <div className="flex flex-col h-full overflow-hidden relative" style={{ background: '#0A0F0C' }}>
      ...
      {showFlash && (
        <div style={{
          ...
          zIndex: 100, opacity: 0.9, transition: 'opacity 0.2s ease', pointerEvents: 'none',
        }} />
      )}

      {/* Media Feed (Camera OR Uploaded Image) */}
      <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
        {cameraMode === 'gallery' && uploadedImageSrc ? (
          <img
            ref={imgRef}
            src={uploadedImageSrc}
            alt="Uploaded from gallery"
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `1.5px solid ${color}`, borderRadius: '6px', background: bgColor, boxShadow: `0 0 12px ${color}30`,
              }}
            >
              {[
              ...
              <div style={{
                position: 'absolute', top: -24, left: -1, background: color, borderRadius: '4px 4px 4px 0', padding: '2px 7px', whiteSpace: 'nowrap',
              }}>
      ...
      <div style={{
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 style={{
              fontFamily: "'DM Serif Display', serif", fontSize: '20px', fontWeight: 400, color: 'white', lineHeight: 1.1, letterSpacing: '-0.3px',
            }}>
              Live Detection
            </h1>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={saveToGallery} style={{
              background: 'rgba(255,255,255,0.10)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '12px', padding: '9px',
            }}>
      ...
      <div style={{
        position: 'absolute', bottom: 0, width: '100%', zIndex: 20, padding: '0 0 16px 0',
        background: 'linear-gradient(to top, rgba(0,0,0,0.75) 0%, transparent 100%)',
      }}>

        {/* Stats + Controls Glass Card */}
        <div style={{
          margin: '0 12px 12px', background: 'rgba(15, 20, 16, 0.72)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)',
          borderRadius: '20px', border: '1px solid rgba(255,255,255,0.08)', padding: '14px 16px',
        }}>
          {/* Live count badges */}
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{
                position: 'absolute', top: '50%', transform: 'translateY(-50%)', left: 0, width: `${((threshold - 10) / 90) * 100}%`,
                height: '4px', borderRadius: '2px', background: 'linear-gradient(90deg, #2D6A4F, #4ADE80)', pointerEvents: 'none', zIndex: 1,
              }} />
              <input type="range" min="10" max="100" value={threshold} onChange={(e) => setThreshold(e.target.value)}
                style={{ width: '100%', height: '4px', appearance: 'none', WebkitAppearance: 'none', background: 'rgba(255,255,255,0.10)', borderRadius: '2px', outline: 'none', cursor: 'pointer', position: 'relative', zIndex: 2 }}
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center" style={{ marginBottom: '12px' }}>
          <div style={{
            background: 'rgba(15, 20, 16, 0.65)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '30px', padding: '3px', display: 'flex', gap: '2px',
          }}>
            {['live', 'photo'].map((mode) => (
              <button
                key={mode}
                onClick={() => {
                  ...
                  setUploadedImageSrc(null); // Clear gallery image if user taps live/photo
                  setDetections([]);
                }}
                style={{
                  padding: '6px 20px', borderRadius: '26px', fontSize: '10px', fontWeight: 700, letterSpacing: '0.8px', textTransform: 'uppercase', transition: 'all 0.25s ease',
                  background: cameraMode === mode ? 'white' : 'transparent',
                  color: cameraMode === mode ? '#0A0F0C' : 'rgba(255,255,255,0.50)',
                  boxShadow: cameraMode === mode ? '0 2px 10px rgba(0,0,0,0.20)' : 'none', border: 'none', cursor: 'pointer',
                }}
              >
                {mode}
              </button>
        ...
        <div className="flex justify-between items-center" style={{ padding: '0 32px' }}>
          ...
          {cameraMode === 'gallery' ? (
             ...
             style={{
               ...
               background: 'rgba(255,255,255,0.15)', border: '2.5px solid rgba(255,255,255,0.55)',
               display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
             }}
           >
            ...
            <button
              ...
              style={{
                position: 'relative', width: 68, height: 68, borderRadius: '50%', background: 'rgba(255,255,255,0.15)', border: '2.5px solid rgba(255,255,255,0.55)',
            ...
            >
              <div style={{
                width: 46, height: 46, borderRadius: isScanning ? '8px' : '50%', background: isScanning ? '#ef4444' : 'linear-gradient(135deg, #2D6A4F, #4ADE80)',
                transition: 'all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)', transform: isScanning ? 'scale(0.6)' : 'scale(1)',
          ...
          {/* Flip Camera (Hidden in Gallery mode) */}
          {cameraMode === 'gallery' ? (
            <div style={{ width: 44, height: 44 }} /> // Empty space to keep layout balanced
          ) : (
            <button
              onClick={() => setCameraFacing(f => f === 'environment' ? 'user' : 'environment')}
              style={{
                width: 44, height: 44, borderRadius: '50%', background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.12)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
              }}
            >
      ...
      {isScanning && cameraMode === 'live' && (
        <div style={{
          position: 'absolute', inset: 0, zIndex: 5, pointerEvents: 'none', backgroundSize: '48px 48px',
